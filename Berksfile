site :opscode

metadata

cookbook 'collectd', github: 'EfrainOlivares/chef-collectd', ref: 'ec50609ed6eb193e0411f30aced91befa571940f'
cookbook 'application_ruby', github: 'poise/application_ruby', ref: '6619b639032b40522f938ede11d06ea7f2584d0e'
cookbook 'rightscale_tag', github: 'rightscale-cookbooks/rightscale_tag'
cookbook 'rsc_ruby', path: '../rsc_ruby'
cookbook 'passenger_apache2', path: '../passenger_apache2'
cookbook 'rest_connection', github: 'rightscale-blueprints/chef', rel: 'cookbooks/rest_connection'
#cookbook 'rightscale' , github: 'rightscale/rightscale_cookbooks',rel: "cookbooks/rightscale"
#cookbook "driveclient", github: 'rightscale/rackspace_cookbooks',rel: "driveclient"
#cookbook "cloudmonitoring",github: 'rightscale/rackspace_cookbooks',rel: "cloudmonitoring"
